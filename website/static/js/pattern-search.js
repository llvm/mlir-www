class MLIRSearch {
    constructor(data) {
        ...
        this.github_base_url = data.github_base_url || 'https://github.com/llvm/llvm-project/blob/main/';
    }

    getNamespaces() {
        try {
            const namespaces = new Set();
            Object.values(this.operations).flat().forEach(entry => {
                if (entry.namespace) {
    ...
    searchOperations(query, filters = {}) {
        try {
            ...
            if (query && query.trim()) {
                ...
                try {
                    ...
                    if (/[.*+?^${}()|[\]\\]/.test(trimmedQuery)) {
                        ...
                        operations = operations.filter(op => op.toLowerCase().includes(lowerQuery));
                    }
                } catch (regexError) {
                    console.warn('Invalid regex pattern, falling back to string matching:', regexError.message);
            ...
            // Apply filters
            const results = operations.map(operationName => {
                let entries = this.operations[operationName];

                // Filter by namespace
                if (filters.namespace) {
                    entries = entries.filter(entry => entry.namespace === filters.namespace);
                }
    ...
    getClassDetails(className) {
        try {
            ...
            const methods = Array.from(methodsMap.entries()).map(([method, operations]) => ({
                method,
                operations: Array.from(operations).sort()
            })).sort((a, b) => a.method.localeCompare(b.method));

            const totalOperations = methods.reduce((sum, methodInfo) => sum + methodInfo.operations.length, 0);
    ...
    generateGitHubURL(className) {
        // Since we don't have class file path info in the JSON, always use GitHub search
...
window.MLIRPatternSearchUI = {
    buildDetailTitleHTML: function(className, githubURL) {
        return className +
            '<a href="' + githubURL + '" target="_blank" rel="noopener" class="github-link" style="margin-left: 10px; font-size: 12px; color: rgba(255,255,255,0.8);">' +
                '(GitHub)' +
            '</a>';
    },
    ...
    buildStatsHTML: function(details) {
        let html = 
            '<div class="detail-stat">' +
                '<div class="detail-stat-value">' + details.totalMethods + '</div>' +
                '<div class="detail-stat-label">Methods</div>' +
            '</div>' +
            '<div class="detail-stat">' +
                '<div class="detail-stat-value">' + details.totalOperations + '</div>' +
        ...
        if (details.namespace) {
            html += 
                '<div class="detail-stat">' +
                    '<div class="detail-stat-value" style="font-size: 12px; font-family: monospace;">' + details.namespace + '</div>' +
    ...
    buildMethodsHTML: function(methods) {
        return methods.map(function(methodInfo) {
            const operationsHTML = methodInfo.operations.map(function(op) {
                return '<div class="operation-tag" onclick="searchForOperation(this.textContent)" title="Click to search for this operation">' +
                    op +
                '</div>';
            }).join('');
            ...
            return '<div class="method-item">' +
                '<div class="method-header">' +
                    ...
                    '<span class="operation-count">' + methodInfo.operations.length + '</span>' +
                '</div>' +
    ...
    buildResultsHeaderText: function(resultsLength, totalEntries, query) {
        if (resultsLength === 0) {
            return query ? ('No results found for "' + query + '"') : 'No operations found';
        }
