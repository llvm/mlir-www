...
function updateStats() {
    try {
        ...
        document.getElementById('stats').textContent =
            stats.totalOperations + ' operations, ' + stats.totalClasses + ' classes, ' + stats.namespaces + ' namespaces, ' + stats.methods + ' methods';
    } catch (error) {
...
function updateAutocomplete(query) {
    ...
    try {
        ...
        // Build dropdown HTML
        dropdown.innerHTML = matches.map((op, index) => {
            const entryCount = searchEngine.operations[op].length;
            return '<div class="autocomplete-item" data-operation="' + op + '" data-index="' + index + '">' +
                    '<div class="operation-name">' + op + '</div>' +
                    '<span class="operation-count">' + entryCount + ' ' + (entryCount === 1 ? 'entry' : 'entries') + '</span>' +
                '</div>';
        }).join('');
        ...
        console.error('Failed to update autocomplete:', error);
        hideAutocomplete();
    }
}
    ...
    switch (e.key) {
        ...
        case 'Enter':
            ...
            if (selectedAutocompleteIndex >= 0 && selectedAutocompleteIndex < items.length) {
                const selectedOperation = items[selectedAutocompleteIndex].dataset.operation;
...
function performSearch() {
    try {
        ...
        document.getElementById('results').innerHTML = 
            '<div class="no-results">Search failed: ' + error.message + '</div>';
    }
}
...
function displayResults(results, query) {
    try {
        ...
        const totalEntries = results.reduce((sum, result) => sum + result.matchCount, 0);
        resultsHeader.textContent = window.MLIRPatternSearchUI.buildResultsHeaderText(results.length, totalEntries, query);
        
        if (results.length === 0) {
            resultsContainer.innerHTML = '<div class="no-results">No matching operations found. Try adjusting your search or filters.</div>';
            return;
        }

        resultsContainer.innerHTML = results.map(result => createResultHTML(result)).join('');
    } catch (error) {
        console.error('Failed to display results:', error);
        document.getElementById('results').innerHTML = 
            '<div class="no-results">Error displaying results: ' + error.message + '</div>';
    }
}
...
function createResultHTML(result) {
    try {
        return '<div class="result-item">' +
                '<div class="operation-name">' + result.operationName + '</div>' +
                result.entries.map(entry => createEntryHTML(entry)).join('') +
            '</div>';
    } catch (error) {
        console.error('Failed to create result HTML:', error);
        return '<div class="result-item"><div class="no-results">Error creating result</div></div>';
    }
}

function createEntryHTML(entry) {
    try {
        const escapedClassName = entry.className.replace(/'/g, "\\'").replace(/"/g, '\\"');
        let html = '<div class="class-entry">' +
                '<div class="class-name" style="text-align: left;">' +
                    '<span class="value" style="margin-right: 10px;">' + entry.method + '</span>' +
                    '<a href="#" onclick="showClassDetails(\'' + escapedClassName + '\'); return false;">' +
                        entry.className +
                    '</a>' +
                    '<a href="' + searchEngine.generateGitHubURL(entry.className) + '" target="_blank" rel="noopener" class="github-link">' +
                        '(GitHub)' +
                    '</a>' +
                '</div>' +
        ...
        if (entry.operations.length > 1) {
            html += '<div class="detail-item">' +
                        ...
                        '<div class="operations-list">' +
                            entry.operations.map(op => '<span class="value">' + op + '</span>').join('') +
                        '</div>' +
                    '</div>';
        }
        ...
        console.error('Failed to create entry HTML:', error);
        return '<div class="class-entry"><div class="no-results">Error creating entry</div></div>';
    }
}
...
function navigateToSearch(query = '', namespace = '', method = '') {
    try {
        ...
        document.getElementById('results').innerHTML = 
            '<div class="no-results">Navigation failed: ' + error.message + '</div>';
    }
}
...
function navigateToDetail(className) {
    try {
        ...
        document.getElementById('detailTitle').innerHTML = window.MLIRPatternSearchUI.buildDetailTitleHTML(
            className, 
            searchEngine.generateGitHubURL(className)
        );
        document.getElementById('detailStats').innerHTML = window.MLIRPatternSearchUI.buildStatsHTML(details);
        document.getElementById('methodsList').innerHTML = window.MLIRPatternSearchUI.buildMethodsHTML(details.methods);
        ...
        document.getElementById('detailContainer').innerHTML = 
            '<div class="no-results">Detail view failed: ' + error.message + '</div>';
    }
}
...
function navigateToSearchInitial() {
    try {
        ...
        console.error('Failed to navigate to initial search:', error);
        document.getElementById('results').innerHTML = 
            '<div class="no-results">Initial search failed: ' + error.message + '</div>';
    }
}
...
async function startApp() {
    try {
        ...
        if (className) {
            ...
            const state = { view: 'detail', className };
            history.replaceState(state, 'MLIR Patterns - ' + className, window.location.toString());
        } else {
            // Show search results
            navigateToSearchInitial();
        }
        ...
        document.getElementById('results').innerHTML = 
            '<div class="no-results">Application failed to start: ' + error.message + '</div>';
    }
}
